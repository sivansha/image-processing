...
def find_best_hit(next_block, search_area, step, cost_func, p):
    iLen, jLen = search_area.shape
    posX, posY = 0, 0
    min_cost = np.inf
    best_hit = None
    ...
    for i in range(0, iLen):
        for j in range(0, jLen):
            diff = next_block - search_area[i:i+step, j:j+step]
            t = cost_func(diff) 
            if(t < min_cost):
                posX, posY = i, j
                min_cost = t
                best_hit = search_area[i:i+step, j:j+step]
    
    return best_hit, (posX, posY)

def EstMotionOld(next, current, cost_func, step = 16, p = 7):
    # step = 16
...
for i in range(len(list_dir) - 1):
    ...
    # axes[0].imshow(img_YCbCr_current, cmap = 'gray')

    # axes[1].set_xlabel("Entropy = {0:.4f}".format(get_entropy(img_YCbCr_next)))
    # axes[1].imshow(img_YCbCr_next, cmap = 'gray')

    # axes[2].set_xlabel("Entropy = {0:.4f}".format(get_entropy(img_YCbCr_prediction)))
    # x,y,dx,dy = [x[0] for x in ls], [x[1] for x in ls], [x[0] for x in ds], [x[1] for x in ds]
    # axes[2].quiver(y,x,dx,dy)
    # axes[2].imshow(img_YCbCr_prediction, cmap = 'gray')
